<?php
	...
	if (isset($_POST['descargarReporte'])) {
		...
		if (!empty($_POST['filcamsr'])){
		    $filtro[]  = 'nombreCampanya = '.'"'.$_POST['filcamsr'].'"';
		}

		if (!empty($_POST['filvinplasr'])){
	    	$filtro[]  = 'placa = '.'"'.$_POST['filvinplasr'].'"'.' OR vin = '.'"'.$_POST['filvinplasr'].'"';
		}  
		...
		$select = $db->prepare("SELECT * FROM campanyas WHERE $filtro ORDER BY id ASC");
		$select->execute();
		...
		if($totalReg > 0 ) {
			...
			$spreadsheet->getProperties()->setCreator('Excel Automotriz El Salvador') /* Autor */
										 ...
										 ->setTitle('Reporte Excel Automotriz Campañas')
										 ->setSubject('Reporte Excel Automotriz Campañas')
										 ->setDescription('Reporte de Campañas')
										 ->setKeywords('reporte campanyas')
										 ->setCategory('Reporte');

			$tituloReporte = "Reporte de Campañas";
			$titulosColumnas = array('ID', 'País', 'VIN', 'Placa', 'Marca', 'Modelo', 'ID Excel', 'ID Fábrica', 'Campaña', 'Descripción');
			
			//Se combinan las celdas de B2 -> K2
			$spreadsheet->setActiveSheetIndex(0)->mergeCells('B2:K2');
			...
			$spreadsheet->setActiveSheetIndex(0)
							->setCellValue('M4', 'Fecha y Hora')
							->setCellValue('N4', date('d-m-Y h:i:s a', time()));
			...
			$spreadsheet->getActiveSheet()->getStyle('B2:K2')->applyFromArray($estiloTituloReporte);
			$spreadsheet->getActiveSheet()->getStyle('B4:K4')->applyFromArray($estiloTituloColumnas);		
			$spreadsheet->getActiveSheet()->getStyle('B5:K'.($i-1))->applyFromArray($estiloInformacion);
			
			$spreadsheet->getActiveSheet()->getStyle('M4')->applyFromArray($estiloTituloReporte);
			$spreadsheet->getActiveSheet()->getStyle('N4')->applyFromArray($estiloTituloColumnas);
			...
			$spreadsheet->getActiveSheet()->setTitle('Campañas');
			...
			header('Content-Disposition: attachment;filename="Reporte de Campañas Excel Recall - '.date("dmY").'.xlsx"');
			...
			header('Location: ../reportes?r=campanyas&msjcls9=mensaje-error&mensaje9=<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> ¡La consulta no ha devuelto ningún valor!#msjcmp');
			
			/* print_r($select); */
		}

	} else {
		header('Location: ../reportes?r=campanyas&msjcls9=mensaje-error&mensaje9=<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> ¡Ha ocurrido algún error!#msjcmp');
		exit;
	}
